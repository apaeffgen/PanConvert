...
BUNDLE_PANDOC="$(which pandoc 2>/dev/null || echo "")"

# ── Pre-flight checks ──
if [ -z "$BUNDLE_PANDOC" ]; then
    echo "[✗] pandoc not found in PATH. Install it or set PATH to include pandoc."
    exit 1
fi
...
PKG_VERSION=$(python3 -c "import sys; sys.path.insert(0, '$PROJECT_DIR'); from source.language.messages import versionnumber; print(versionnumber)")
OUTPUT_PKG="$DIST_DIR/Panconvert-${PKG_VERSION}-macos.pkg"
...
pkg = Packages(
    ...
    pkg_file_structure={
        ...
        '$BUNDLE_PANDOC': '/usr/local/bin/pandoc',
    },
)
ok = pkg.build()
if not ok:
